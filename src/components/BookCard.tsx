...
export default function BookCard({ book }: BookCardProps) {
    ...
    return (
        <div className="group relative bg-white/5 backdrop-blur-xl border border-white/10 rounded-2xl overflow-hidden transition-all duration-500 hover:border-purple-500/50 hover:bg-white/10 hover:shadow-2xl hover:shadow-purple-500/20 hover:-translate-y-2">
            {/* Book Image */}
            <Link href={`/book/${book.id}`}>
                <div className="relative w-full h-64 bg-slate-800 overflow-hidden">
                    ...
                    {/* Status Badge */}
                    {!book.isAvailable && (
                        <div className="absolute top-4 right-4 px-3 py-1 bg-red-500/90 backdrop-blur-sm rounded-full text-white text-xs font-semibold">
                            Sold
                        </div>
                    )}
                ...
                <div className="flex items-center justify-between pt-4 border-t border-white/10">
                    ...
                    {book.isAvailable && (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg transition-colors font-semibold text-sm"
                        >
                            <svg
                                className="w-5 h-5"
                                fill="currentColor"
